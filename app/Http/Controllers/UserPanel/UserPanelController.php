...
class UserPanelController extends Controller
{
                    ...
                    if (!$coupon->isPercent()) {
                        if (!Auth::user()->hasCouponUsed($coupon)) {
                            if (Auth::user()->redeemCoupon($coupon)) {
                                return redirect()->route('deposit')->with([
                                    'successMessage' => __('frontend/user.coupon_redeem.success', [
        ...
        if ($request->getMethod() == 'POST') {
            $validator = Validator::make($request->all(), [
                ...
                'settings_new_password' => [
                    ...
                    'min:6',
                    'max:255',
                    'same:settings_new_password_confirm',
                ],
            ...
            if (!$validator->fails()) {
                if (Hash::check($request->input('settings_current_password'), Auth::user()->password)) {
                    ...
                    return redirect()->route('settings')->with('successMessageSettingsPassword',
                        __('frontend/user.success_password_changed'));
                } else {
                    $validator->getMessageBag()->add('settings_current_password',
                        __('frontend/user.settings_current_password_wrong'));
        ...
        if ($request->getMethod() == 'POST') {
            $validator = Validator::make($request->all(), [
                'settings_jabber_id' => [
                    ...
                    'email',
                    'max:255',
                    'unique:users,jabber_id,' . Auth::user()->id,
                ],
            ]);
    ...
    public function showOrdersPage($pageNumber = 0)
    {
        ...
        $user_orders = UserOrder::where('user_id', Auth::user()->id)->orderByDesc('created_at')->paginate(1, ['*'],
            'page', $pageNumber);
    ...
    {
        /* die("here"); */
        $user_orders = UserOrder::where('user_id', Auth::user()->id)->orderByDesc('created_at')->paginate(5, ['*'],
            'page', $pageNumber);
    ...
    {
        $user_transactions = UserTransaction::where('user_id', Auth::user()->id)->orderByDesc('created_at')->paginate(5,
            ['*'], 'page', $pageNumber);
    ...
    {
        $myObject = MyObject::where('created_at', '>=', Carbon::now()->subMonth())
            ->orderBy('created_at', 'asc')
            ->get();
                $months = ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'];
        return view('charts',compact('myObject','months'));
    }
}
